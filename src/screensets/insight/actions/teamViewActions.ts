...
export function deriveTeamKpis(members: TeamMember[], period: PeriodValue) {
  ...
  const atRisk = members.filter((m) =>
    TEAM_VIEW_CONFIG.alert_thresholds.some((t) => {
      const val = m[t.metric_key as keyof TeamMember];
      return val !== null && val !== undefined && (val as number) < t.trigger;
    }),
  ).length;
  ...
  return (TEAM_KPIS_BY_PERIOD[period] ?? TEAM_KPIS_BY_PERIOD['month']).map((k) => {
    if (k.metric_key === 'at_risk_count') {
      const chipLabel = atRisk === 0 ? 'All on track' : k.chipLabel;
      return { ...k, value: String(atRisk), status: atRiskStatus, chipLabel };
    }
    if (k.metric_key === 'team_dev_time') {
      const devTimeStatus: 'good' | 'warn' | 'bad' = devTimeMedian <= 14 ? 'good' : devTimeMedian <= 20 ? 'warn' : 'bad';
      const companyAvg = 16;
      const chipLabel = devTimeMedian <= companyAvg ? `↓ vs company ${companyAvg}h` : `↑ vs company ${companyAvg}h`;
      return { ...k, value: `${devTimeMedian}h`, status: devTimeStatus, chipLabel };
    }
    if (k.metric_key === 'focus_gte_60') {
      const chipLabel = belowFocus === 0 ? 'All on target' : k.chipLabel;
      return { ...k, value: `${focusCount} / ${total}`, sublabel: `${belowFocus} member${belowFocus !== 1 ? 's' : ''} below target`, status: focusStatus, chipLabel };
    }
    if (k.metric_key === 'not_using_ai') {
      const chipLabel = noAiCount === 0 ? 'All using AI' : 'Action needed';
      const sublabel  = noAiCount === 0 ? 'All members active this period' : k.sublabel;
      return { ...k, value: String(noAiCount), status: noAiStatus, chipLabel, sublabel };
    }
    return k;
  });
}
  ...
  void Promise.allSettled([
    ...
    .then(([membersResult, deliveryResult, qualityResult, collabResult, aiResult, availResult]) => {
      ...
      const data: TeamViewData = {
        teamName,
        teamKpis:      [],  // computed in TeamViewScreen from filtered members
        members,
        bulletSections,
