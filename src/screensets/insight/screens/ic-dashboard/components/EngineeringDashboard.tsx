/**
 * EngineeringDashboard — IC dashboard body for engineering personas.
 *
 * Extracted from `IcDashboardScreen` so the page component can branch on
 * `_identity.department` between this (developers / R&D) and `SalesDashboard`
 * (sales reps) without growing into a giant if/else. Behaviour is unchanged
 * from the pre-refactor screen; only the i18n loader stays at the page
 * level so it registers exactly once per dashboard mount.
 */
...
const EngineeringDashboard: React.FC<EngineeringDashboardProps> = ({ personId }) => {
